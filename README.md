...
It allows you to create different resumes in different languages, or with different
styles and foci.
...
```bash
git clone https://github.com/riccardo-giomi/rails8-mycv
```
...
allows you to change section titles and set page break points that will be used by
...
**Note on adding and removing sections**: adding or removing sections like
...
required for the current CV or to better reflect the CV's purpose.

The form also offers some control over the position of page breaks when
printing or exporting the CV as PDF.

### Preview

A preview page can be opened in a new tab or page; the preview will auto-refresh
on changes to the CV data.

This page has no navigation bar or controls, but of course
`Print` is available from the browser's menu.

Custom page breaks will be shown as dashed lines in the preview, but those will
not appear if the preview page is printed.
...
The application allows you to import/export CV data as JSON files from the command-line.
...
CRUD for Cover Letters, with functionalities similar to those the CVs have.
